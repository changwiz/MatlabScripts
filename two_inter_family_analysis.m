function two_inter_family_analysis()
PATH = '/Users/wentaochang/Documents/BARS_Analysis/InterFamily/';
family_file = strcat(PATH,'inter_analysis_family_two.txt');
time_file = strcat(PATH,'inter_analysis_time.txt');
%duration_file = strcat('./',family,'_duration_stat.txt');
volume_file = strcat(PATH,'inter_analysis_translate_two.txt');
family_ip_file = strcat(PATH,'inter_analysis_concurrent_bot_two.txt');
...
for j = 1:size(ip_list,2)
    ...
    h = scatter3(data(:,1),data(:,2),data(:,3),36,'fill','MarkerEdgeColor','k');
    hold on;
end
hold on;
...
saveas(hFig,strcat(PATH,'inter_analysis_two.eps'),'epsc');
